...
/* ─── Agent Graph ─── */
function AgentGraph({ agents, edges, selectedId, runningIds, onSelect }: {
  agents: Agent[]; edges: GraphEdge[]; selectedId: string | null
  runningIds: Set<string>; onSelect: (id: string) => void
}) {
        ...
        <svg className="graph__svg" width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
          ...
          {agents.map(a => {
            ...
            return (
              ...
              >
                {isRunning && <rect className="gnode__pulse" x={-6} y={-6} width={w+12} height={h+12} rx={4} fill="none" stroke="var(--accent)" strokeWidth="1.5"/>}
...
/* ─── List view ─── */
function AgentListView({ agents, selectedId, runningIds, onSelect }: {
  agents: Agent[]; selectedId: string | null; runningIds: Set<string>; onSelect: (id: string) => void
}) {
  return (
    <div className="alist">
            ...
            <li key={a.id} className={`alist__row${selectedId === a.id ? ' alist__row--active' : ''}`} onClick={() => onSelect(a.id)}>
              <span className="alist__cell">
                <div style={{ position:'relative', display:'inline-flex' }}>
                  ...
                  {runningIds.has(a.id) && <span className="agent-running-dot"/>}
                </div>
                <span>{a.name}</span>
              </span>
...
function AgentConfig({ agent, agents, skills, isRunning, onUpdate, onSave, onDelete, onStop, onClose }: {
  agent: Agent; agents: Agent[]; skills: SkillRead[]; isRunning: boolean
  onUpdate: (patch: Partial<Agent>) => void
  onSave: (agent: Agent) => Promise<void>
  onDelete: () => void; onStop: () => void; onClose: () => void
}) {
  ...
  const stopNow = async () => {
    setStopping(true)
    try { await agentsApi.stop(agent.id); onStop() }
    ...
    <aside className="agent-cfg">
      ...
      <footer className="agent-cfg__foot">
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          ...
          {isAI && isRunning && (
            <button className="btn btn--sm btn--danger" onClick={stopNow} disabled={stopping}>
  ...
  useEffect(() => {
    const refresh = () => {
      runsApi.list().then(runs => {
        setRunningIds(new Set(runs.filter(r => r.status === 'running').map(r => r.agent_id)))
      }).catch(() => {})
    }
    refresh()
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', minHeight:0 }}>
      ...
      <div className="agents" style={{ flex:1 }}>
        ...
        <section className="agents__canvas" style={{ position: 'relative' }}>
          {view === 'graph' ? (
            <AgentGraph agents={agents} edges={edges} selectedId={selectedId} runningIds={runningIds} onSelect={id => setSelectedId(i => i === id ? null : id)} />
          ) : (
            <AgentListView agents={filtered} selectedId={selectedId} runningIds={runningIds} onSelect={id => setSelectedId(i => i === id ? null : id)} />
          )}
          {error && (
        ...
        {selected && (
          <AgentConfig
            ...
            onStop={() => setRunningIds(ids => { const s = new Set(ids); s.delete(selected.id); return s })}
